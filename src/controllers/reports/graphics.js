...
crtlGraphics.findDate = async (req, res) => {
  ...
  if (!token) {
    return res
      .status(401)
      .json({ message: "No se encuentra autorizado para solicitar recursos " });
  }
  try {
    ...
    if (!reports) {
      return res
        .status(404)
        .json({
          message: "no hay informes cargados en las fecha especificadas!",
        });
    }
...
crtlGraphics.findTitle = async (req, res) => {
  try {
    const title = await Informe.findAll({
      attributes: [
        "Titulo",
        [sequelize.fn("COUNT", sequelize.col("Titulo")), "count"],
      ],
      group: ["Titulo"],
