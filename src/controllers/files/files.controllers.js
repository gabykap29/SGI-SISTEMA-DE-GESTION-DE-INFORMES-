...
ctrlFiles.create = async (req, res) => {
  ...
  try {
    ...
    if (informe.Files.length > 4) {
      return res
        .status(400)
        .json({ message: "Solo se pueden subir 4 imagenes!" });
    }
    ...
    if (!newFile) {
      return res
        ...
        .json({
          message:
            "Error al subir la imagem, verifique que la imagen tenga un formato correcto!",
        });
    }
  ...
  try {
    ...
    if (!fileDeleted && !deleted) {
      return res
        ...
        .json({
          message:
            "Error al eliminar la imagen o archivo! Comuniquese con su Administrador!",
        });
    }
  } catch (error) {
    console.log(error);
